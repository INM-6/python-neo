...
class TestNestIO_Spiketrains(BaseTestIO, unittest.TestCase):
    ...
    def test_read_spiketrain(self):
        ...
        r = NestIO(filenames=filename)
        r.read_spiketrain(
            gdf_id=1, t_start=400.0 * pq.ms, t_stop=500.0 * pq.ms, lazy=False, id_column_gdf=0, time_column_gdf=1
        )
        r.read_segment(
    ...
    def test_read_spiketrain_annotates(self):
        ...
        r = NestIO(filenames=filename)
        ID = 7
        st = r.read_spiketrain(gdf_id=ID, t_start=400.0 * pq.ms, t_stop=500.0 * pq.ms)
    ...
    def test_read_segment_annotates(self):
        ...
        r = NestIO(filenames=filename)
        IDs = (5, 11)
        sts = r.read_segment(gid_list=(5, 11), t_start=400.0 * pq.ms, t_stop=500.0 * pq.ms)
    ...
    def test_adding_custom_annotation(self):
        ...
        filename = self.get_local_path("nest/0gid-1time-1256-0.gdf")
        r = NestIO(filenames=filename)
        st = r.read_spiketrain(gdf_id=0, t_start=400.0 * pq.ms, t_stop=500.0 * pq.ms, layer="L23", population="I")
    ...
    def test_t_start_t_stop(self):
        ...
        seg = r.read_segment(
            gid_list=[], t_start=t_start_targ, t_stop=t_stop_targ, lazy=False, id_column_gdf=0, time_column_gdf=1
        )
        sts = seg.spiketrains
        self.assertTrue(
    ...
    def test_t_start_undefined_raises_error(self):
        ...
        r = NestIO(filenames=filename)
        with self.assertRaises(ValueError):
            r.read_spiketrain(gdf_id=1, t_stop=500.0 * pq.ms, lazy=False, id_column=0, time_column=1)
        with self.assertRaises(ValueError):
    ...
    def test_t_stop_undefined_raises_error(self):
        ...
        r = NestIO(filenames=filename)
        with self.assertRaises(ValueError):
            r.read_spiketrain(gdf_id=1, t_start=400.0 * pq.ms, lazy=False, id_column=0, time_column=1)
        with self.assertRaises(ValueError):
    ...
    def test_gdf_id_illdefined_raises_error(self):
        ...
        r = NestIO(filenames=filename)
        with self.assertRaises(ValueError):
            r.read_spiketrain(gdf_id=[], t_start=400.0 * pq.ms, t_stop=500.0 * pq.ms)
        with self.assertRaises(ValueError):
            r.read_spiketrain(gdf_id=[1], t_start=400.0 * pq.ms, t_stop=500.0 * pq.ms)
        with self.assertRaises(ValueError):
    ...
    def test_read_segment_can_return_empty_spiketrains(self):
        ...
        filename = self.get_local_path("nest/0gid-1time-1256-0.gdf")
        r = NestIO(filenames=filename)
        seg = r.read_segment(gid_list=[], t_start=400.4 * pq.ms, t_stop=400.5 * pq.ms)
    ...
    def test_read_spiketrain_can_return_empty_spiketrain(self):
        ...
        filename = self.get_local_path("nest/0gid-1time-1256-0.gdf")
        r = NestIO(filenames=filename)
        st = r.read_spiketrain(gdf_id=0, t_start=400.0 * pq.ms, t_stop=410.0 * pq.ms)
        self.assertEqual(st.size, 0)
